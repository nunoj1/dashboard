<script lang="ts">
	...
	import PriorityDot from '$lib/components/ui/PriorityDot.svelte';
	import UrgencyBadge from '$lib/components/ui/UrgencyBadge.svelte';
	import SubtaskList from './SubtaskList.svelte';
	import { formatDueDate, formatShortDate, getUrgency, urgencyBorder, type Urgency } from '$lib/utils/date';
	...
	let { todo, ontoggle, onconfirmDone, onsubtaskToggle, onsubtaskDelete } = $props<{
		todo: Todo;
...
<div class="border border-zinc-800/50 rounded-lg overflow-hidden transition {urgencyBorder(urgency)} border-l-4">
	<div class="flex items-center gap-3 bg-zinc-950/50 px-4 py-3">
		...
		<div class="flex-1 min-w-0 cursor-pointer" onclick={() => hasDetails && (expanded = !expanded)} role="button" tabindex="0" onkeydown={(e) => e.key === 'Enter' && (expanded = !expanded)}>
		...
		<PriorityDot priority={todo.priority ?? 'medium'} />
